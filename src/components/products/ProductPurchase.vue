    ...
    <div class="row">
        <div class="col-6 offset-3 pt-3 card mt-5 shadow">
            <div class="card-body">
                ...
                <hr>
                <button @click="saveProduct" :disabled="saveDisabled" class="btn btn-primary">Kaydet</button>
            </div>
        </div>
    </div>
</div>
...
export default {
    ...
    beforeRouteLeave(to, from, next) {
        if ((this.product.title.length > 0 || this.product.count > 0 || this.product.description.length > 0 || this.product.price > 0) && !this.saveButtonClicked) {
